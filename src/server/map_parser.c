...
static void map_load_header_value(GScanner *scan, conf_t *conf)
{
  ...
  switch (map_hdr[i].type)
    {
    case G_TOKEN_INT:
      G_STRUCT_MEMBER(int, conf, map_hdr[i].offs) = parser_get_int(scan);
      break;
    case G_TOKEN_FLOAT:
      G_STRUCT_MEMBER(float, conf, map_hdr[i].offs) = parser_get_float(scan);
      break;
    case G_TOKEN_STRING:
      G_STRUCT_MEMBER(char *, conf, map_hdr[i].offs) = parser_get_string(scan);
      break;
    default:
      g_scanner_error(scan, "Fonction non implementée.\n");
      abort();      
    };
...
static void map_load_header(GScanner *scan, conf_t *conf)
{
  ...
  if (strcmp(g_scanner_cur_value(scan).v_identifier, "header"))
    {
      g_scanner_error(scan, "Section [header] non trouvée.\n");
      abort();
    }
  ...
  if (strcmp(g_scanner_cur_value(scan).v_identifier, "objects"))
    {
      g_scanner_error(scan, "Section [objects] non trouvée.\n");
      abort();
    }
  parser_check_type(scan, g_scanner_get_next_token(scan), G_TOKEN_RIGHT_BRACE);
}
